...
```powershell
node .\bin\c2c.js write-probe-status -w D:\python\codex-with-chatgpt --json
```
...
1. 在 PowerShell 中进入仓库并启用探针。仅在源码或 Skill 更新后，先运行 `powershell -NoProfile -File .\scripts\dev-install.ps1` 部署：
   ...
   node .\bin\c2c.js restart -w D:\python\codex-with-chatgpt --tunnel
   node .\bin\c2c.js write-probe-status -w D:\python\codex-with-chatgpt --json
   ```
...
3. 打开 [ChatGPT Plugins](https://chatgpt.com/plugins)，找到已有的 **Codex with ChatGPT · codex-with-chatgpt** 连接并点 **Refresh**；确认工具列表出现 `write_probe`、`readOnlyHint=false` 后，在新对话中选择该连接。若缺少 `probe.write`，用 `node .\bin\c2c.js pair -w D:\python\codex-with-chatgpt` 生成一次性配对码，并按授权页重新授权；确认同意页包含“C2C 状态写入实验”。旧 refresh token 不会自动扩权；不要自动删除 connector、unpair、重做 tunnel 或 project。
...
4. 真实 `write_probe` 调用由用户在新对话中手工发送。可直接发送：
   ...
   然后在本地再次运行 `node .\bin\c2c.js write-probe-status -w D:\python\codex-with-chatgpt --json`，核对 nonce 与本次测试值一致、timestamp 是本次调用时间、workspaceId 属于当前工作区。只有实际落盘才算完成调用验证：
