...
describe("MCP write_probe", () => {
  it("默认关闭时列出9个工具，开启时恰增write_probe且动态advertise probe.write", async () => {
    ...
    await withBridge(false, async ({ bridge, connect }) => {
      ...
      expect(await discoveryScopes(bridge.localBaseUrl(), "/.well-known/oauth-authorization-server/mcp")).toEqual(BASE_SCOPES);
      expect(await discoveryScopes(bridge.localBaseUrl(), "/.well-known/oauth-protected-resource/mcp")).toEqual(BASE_SCOPES);
    });
