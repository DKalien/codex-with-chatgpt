...
describe.each(["LF", "CRLF"])("Activation Skill 文本契约（%s）", style => {
  ...
  it("reuse/use_remote 不 bind-current；bind_current 是本地先决步骤", () => {
    ...
    const openBlock = activation.split("- **`open_project_chat`**：")[1]?.split("- **`bind_current`**")[0] ?? "";
    ...
    expect(bindBlock).toContain("本机 CLI、Desktop IPC 和本机确认窗");
    expect(bindBlock).toContain("不要等待尚不存在的 ChatGPT conversation 或");
    expect(bindBlock).toContain("不打开浏览器、不创建或进入 Project/chat");
    expect(bindBlock).toContain("不会自动打开 Project/chat");
    expect(bindBlock).toContain("Browser Companion feedback");
    expect(bindBlock).toContain("plane 是独立可选的回写路径");
    expect(bindBlock).not.toContain("bind_current` → `open_project_chat`");
    expect(openBlock).toContain("显式 ChatGPT 导航 action");
    expect(openBlock).toContain("threadConversation.projectUrl");
